"""
Land-cover feature engineering for historical FIRMS training data.

Uses ESA WorldCover 2021 raster tiles to assign a land-cover class
to every India-only FIRMS training observation.
"""
...
INPUT_FILE = DATA_INTERIM / "firms_history_india.csv"

OUTPUT_FILE = DATA_INTERIM / "training_hotspots_with_landcover.csv"
...
def sample_training_landcover():

    # Load India-only historical FIRMS data
    df = pd.read_csv(INPUT_FILE)

    print("==========================================")
    print("BUILDING TRAINING LAND-COVER FEATURES")
    print("==========================================")
    ...
    # Save training land-cover dataset
    df.to_csv(
        OUTPUT_FILE,
        index=False,
    )
    ...
    print()
    print(f"Unknown/missing: {(df['landcover_class'] == 'unknown').sum()}")
